...
get_answer = st.session_state.functions['get_answer']
...
# Chat input
if st.session_state["prompt_count"] < 5 and (prompt := st.chat_input("Ask about products, prices, or stock")):
    st.session_state["messages"].append({"role": "user", "content": prompt})
    st.chat_message("user").write(prompt)

    try:
        response = get_answer(prompt, user_id, token)
...
if st.session_state["prompt_count"] >= 5:
    st.warning("You've reached your 5-question limit. Please refresh the page to restart.")
